...
const ReservatePage = () => {
    ...
    function isDateValid(selectedDate) {
        // Chuyển đổi selectedDate sang đối tượng Date
        const date = new Date(selectedDate);

        // Lấy ngày hiện tại và đặt thời gian về 00:00:00 để chỉ xét ngày
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        // Kiểm tra nếu ngày được chọn >= ngày hôm nay
        return date >= today;
    }
    const handleButtonReservateClick = async () => {
        ...
        if (isDateValid(selectedDate) === false) {
            message.error('Ngày đặt không hợp lệ!')
            return;
        }

        const totalAmount = calculateTotal(selectedFoods);
        if (totalAmount === 0) {
            message.error('Vui lòng chọn món trước!');
            return;
        }
